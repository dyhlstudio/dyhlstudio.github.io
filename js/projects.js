...
function fontScrambler() {
    var fonts = ["Gulax Regular", "iA Writer Mono", "stratos", "brevier", "Fluxisch Else Bold", "Ortica Light", "freight-display-pro"];
    ...
    for (i = 0; i < projectsList[selectionNo].assets.length; i++) {
        ...
        if (projectsList[selectionNo].frame[i] === "3d") {
            if (i == 0) {
                slides.push(
                    e('div', { key: i, className: 'd-flex justify-content-center align-items-center frame inactive active' },
                        e('model-viewer', { className: 'threed', src: projectsList[selectionNo].assets[i], shadow-intensity: "1", auto-rotate: null, camera-controls: null })
                    )
                );
            } else {
                slides.push(
